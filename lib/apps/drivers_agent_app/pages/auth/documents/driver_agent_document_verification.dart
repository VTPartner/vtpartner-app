...
class DriverAgentDocumentVerificationScreen extends StatefulWidget {
  const DriverAgentDocumentVerificationScreen({super.key});

  @override
  State<DriverAgentDocumentVerificationScreen> createState() =>
      _DriverAgentDocumentVerificationScreenState();
}

class _DriverAgentDocumentVerificationScreenState
    extends State<DriverAgentDocumentVerificationScreen> {
...
//     };
  ...
  Future<void> saveDriverDetails() async {
    ...
    var driver_agent_aadhar_front_photo_url =
        pref.getString("driver_agent_aadhar_front_photo_url");
    var driver_agent_aadhar_back_photo_url =
        pref.getString("driver_agent_aadhar_back_photo_url");
    var driver_agent_aadhar_no = pref.getString("driver_agent_aadhar_no");
    var driver_agent_license_front_photo_url =
        pref.getString("driver_agent_license_front_photo_url");
    var driver_agent_license_back_photo_url =
        pref.getString("driver_agent_license_back_photo_url");
    var driver_agent_license_no = pref.getString("driver_agent_license_no");
    var driver_agent_pan_no = pref.getString("driver_agent_pan_no");
    var driver_agent_pan_front_photo_url =
        pref.getString("driver_agent_pan_front_photo_url");
    var driver_agent_pan_back_photo_url =
        pref.getString("driver_agent_pan_back_photo_url");
    var driver_agent_selfie_photo_url =
        pref.getString("driver_agent_selfie_photo_url");
    ...
    if (driver_agent_license_front_photo_url == null ||
        driver_agent_license_front_photo_url.isEmpty) {
      glb.showToast("Please upload Driving License Front Picture");
      return;
    }
    if (driver_agent_license_back_photo_url == null ||
        driver_agent_license_back_photo_url.isEmpty) {
      glb.showToast("Please upload Driving License Back Picture");
      return;
    }
    if (driver_agent_license_no == null || driver_agent_license_no.isEmpty) {
      glb.showToast("Please upload Driving License Number");
      return;
    }
    if (driver_agent_aadhar_front_photo_url == null ||
        driver_agent_aadhar_front_photo_url.isEmpty) {
      glb.showToast("Please upload Aadhar Front Picture");
      return;
    }
    if (driver_agent_aadhar_back_photo_url == null ||
        driver_agent_aadhar_back_photo_url.isEmpty) {
      glb.showToast("Please upload Aadhar Back Picture");
      return;
    }
    if (driver_agent_aadhar_no == null || driver_agent_aadhar_no.isEmpty) {
      glb.showToast("Please provide your Aadhar Number");
      return;
    }
    if (driver_agent_pan_no == null || driver_agent_pan_no.isEmpty) {
      glb.showToast("Please upload PAN Number");
      return;
    }
    if (driver_agent_pan_front_photo_url == null ||
        driver_agent_pan_front_photo_url.isEmpty) {
      glb.showToast("Please upload PAN Front Picture");
      return;
    }
    if (driver_agent_pan_back_photo_url == null ||
        driver_agent_pan_back_photo_url.isEmpty) {
      glb.showToast("Please upload PAN Back Picture");
      return;
    }
    if (driver_agent_selfie_photo_url == null ||
        driver_agent_selfie_photo_url.isEmpty) {
      glb.showToast("Please upload your selfie");
      return;
    }

    pref.setString("driver_agent_driver_name", driverName);
    pref.setString("driver_agent_driver_address", driverAddress);
    pref.setString("driver_agent_driver_city_id", _selectedCityID!);
    pref.setString("driver_agent_driver_gender", _selectedGender!);
