...
class DriverAgentAadharCardUploadScreen extends StatefulWidget {
  const DriverAgentAadharCardUploadScreen({super.key});

  @override
  State<DriverAgentAadharCardUploadScreen> createState() =>
      _DriverAgentAadharCardUploadScreenState();
}

class _DriverAgentAadharCardUploadScreenState
    extends State<DriverAgentAadharCardUploadScreen> {
  File? _aadharFront;
  File? _aadharBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousAadharFront = pref.getString("driver_agent_aadhar_front_photo_url");
    previousAadharBack = pref.getString("driver_agent_aadhar_back_photo_url");
    var driver_agent_aadhar_no = pref.getString("driver_agent_aadhar_no");
    if (driver_agent_aadhar_no != null && driver_agent_aadhar_no!.isNotEmpty) {
      aadharCardNoTextEdit.text = driver_agent_aadhar_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("driver_agent_aadhar_front_photo_url", retUrl);
        } else {
          await pref.setString("driver_agent_aadhar_back_photo_url", retUrl);
        }

        pref.setString("driver_agent_aadhar_no",
            aadharCardNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> saveAadharDetails() async {
    ...
    String? savedAadharNo = pref.getString("driver_agent_aadhar_no");
    previousAadharFront = pref.getString("driver_agent_aadhar_front_photo_url");
    previousAadharBack = pref.getString("driver_agent_aadhar_back_photo_url");
    ...
    if (isNewEntry || savedAadharNo != aadharNo) {
      await pref.setString("driver_agent_aadhar_no", aadharNo);
    }
