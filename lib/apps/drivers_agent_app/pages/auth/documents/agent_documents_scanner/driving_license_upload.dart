...
class DriverAgentDrivingLicenseUploadScreen extends StatefulWidget {
  const DriverAgentDrivingLicenseUploadScreen({super.key});

  @override
  State<DriverAgentDrivingLicenseUploadScreen> createState() =>
      _DriverAgentDrivingLicenseUploadScreenState();
}

class _DriverAgentDrivingLicenseUploadScreenState
    extends State<DriverAgentDrivingLicenseUploadScreen> {
  File? _licenseFront;
  File? _licenseBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousLicenseFront =
        pref.getString("driver_agent_license_front_photo_url");
    previousLicenseBack = pref.getString("driver_agent_license_back_photo_url");
    var driver_agent_license_no = pref.getString("driver_agent_license_no");
    if (driver_agent_license_no != null &&
        driver_agent_license_no!.isNotEmpty) {
      licenseNoTextEdit.text = driver_agent_license_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("driver_agent_license_front_photo_url", retUrl);
        } else {
          await pref.setString("driver_agent_license_back_photo_url", retUrl);
        }

        pref.setString("driver_agent_license_no",
            licenseNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> saveLicenseDetails() async {
    ...
    String? savedLicenseNo = pref.getString("driver_agent_license_no");
    previousLicenseFront =
        pref.getString("driver_agent_license_front_photo_url");
    previousLicenseBack = pref.getString("driver_agent_license_back_photo_url");
    ...
    if (isNewEntry || savedLicenseNo != licenseNo) {
      await pref.setString("driver_agent_license_no", licenseNo);
    }
