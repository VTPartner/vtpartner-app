...
class CabAgentVehicleOwnerDetailsScreen extends StatefulWidget {
  const CabAgentVehicleOwnerDetailsScreen({super.key});

  @override
  State<CabAgentVehicleOwnerDetailsScreen> createState() =>
      _CabAgentVehicleOwnerDetailsScreenState();
}

class _CabAgentVehicleOwnerDetailsScreenState
    extends State<CabAgentVehicleOwnerDetailsScreen> {
  ...
  Future<void> saveOwnerDetails() async {
    ...
    var cab_agent_owner_selfie_photo_url =
        pref.getString("cab_agent_owner_selfie_photo_url");
    ...
    if (cab_agent_owner_selfie_photo_url == null ||
        cab_agent_owner_selfie_photo_url.isEmpty) {
      glb.showToast("Please upload Owner Photo");
      return;
    }

    pref.setString("cab_agent_owner_name", ownerName);
    pref.setString("cab_agent_owner_address", ownerAddress);
    pref.setString("cab_agent_owner_city_name", cityName);
    pref.setString("cab_agent_owner_mobile_no", ownerMobileNo);
    registerDriverAsync();
  ...
  Future<void> registerDriverAsync() async {
    ...
    var cab_agent_owner_name = pref.getString("cab_agent_owner_name");
    var cab_agent_owner_mobile_no = pref.getString("cab_agent_owner_mobile_no");
    var owner_photo_url = pref.getString("cab_agent_owner_selfie_photo_url");
    var cab_agent_owner_address = pref.getString("cab_agent_owner_address");
    var cab_agent_owner_city_name = pref.getString("cab_agent_owner_city_name");
    ...
    final data = {
      ...
      "owner_name": cab_agent_owner_name,
      "owner_mobile_no": cab_agent_owner_mobile_no,
      "owner_photo_url": owner_photo_url,
      "owner_address": cab_agent_owner_address,
      "owner_city_name": cab_agent_owner_city_name
    };
    print("data::$data");
    try {
  ...
  loadDefault() async {
    ...
    var cab_agent_owner_name = pref.getString("cab_agent_owner_name")!;
    var cab_agent_owner_address = pref.getString("cab_agent_owner_address")!;
    var city_name = pref.getString("cab_agent_owner_city_name")!;
    var cab_agent_owner_mobile_no =
        pref.getString("cab_agent_owner_mobile_no")!;

    if (cab_agent_owner_name.isNotEmpty) {
      nameTxtEdit.text = cab_agent_owner_name;
    }
    if (cab_agent_owner_address.isNotEmpty) {
      addressTxtEdit.text = cab_agent_owner_address;
    }
    if (city_name.isNotEmpty) {
      cityNameTxtEdit.text = city_name;
    }
    if (cab_agent_owner_mobile_no.isNotEmpty) {
      mobileNoTxtEdit.text = cab_agent_owner_mobile_no;
    }
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              documentStyle(() {
                Navigator.pushNamed(context, CabAgentOwnerPhotoUploadRoute);
