...
class CabAgentVehicleRCUpload extends StatefulWidget {
  const CabAgentVehicleRCUpload({super.key});

  @override
  State<CabAgentVehicleRCUpload> createState() =>
      _CabAgentVehicleRCUploadState();
}

class _CabAgentVehicleRCUploadState extends State<CabAgentVehicleRCUpload> {
  File? _rcImage;
  String? previousRC;
  String? previousRCNo;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousRC = pref.getString("cab_agent_rc_photo_url");
    previousRCNo = pref.getString("cab_agent_rc_no");
    if (previousRCNo != null && previousRCNo!.isNotEmpty) {
      textInput.text = previousRCNo!;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await pref.setString("cab_agent_rc_photo_url", retUrl);

        glb.showToast('RC image uploaded successfully');
      } else {
  ...
  Future<void> saveRCDetails() async {
    var rcNo = textInput.text.trim();
    SharedPreferences pref = await SharedPreferences.getInstance();

    // Check if Aadhar number is provided
    if (rcNo.isEmpty) {
      glb.showToast("Please provide your RC number");
      return;
    }

    // Fetch existing data from SharedPreferences
    String? savedRCNo = pref.getString("cab_agent_rc_no");
    previousRC = pref.getString("cab_agent_rc_photo_url");

    // Determine if this is a new entry
    bool isNewEntry =
        savedRCNo == null && (previousRC == null || previousRC!.isEmpty);

    // Check if the front and back images need uploading
    bool needsFrontUpload = (isNewEntry || _rcImage != null) &&
        (previousRC == null ||
            _rcImage != null && _rcImage!.path != previousRC);

    // Show a toast if the front or back image is missing on a new entry
    if (isNewEntry) {
      if (_rcImage == null) {
        glb.showToast("Please select and upload your RC image");
        return;
      }
    }

    // Upload front image if necessary
    if (needsFrontUpload && _rcImage != null) {
      await _uploadImage(_rcImage!, true);
    }

    // Update Aadhar number if it's a new entry or has changed
    if (isNewEntry || savedRCNo != rcNo) {
      await pref.setString("cab_agent_rc_no", rcNo);
    }
    ...
    if ((isNewEntry && previousRC != null) ||
        needsFrontUpload ||
        savedRCNo != rcNo) {
      glb.showToast("RC details saved successfully");
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        "Upload Registration Certificate[ RC ] Details ",
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: (previousRC != null && _rcImage == null)
                                ? DecorationImage(
                                    image: NetworkImage(previousRC!),
                                    fit: BoxFit.contain,
                                  )
                                : _rcImage != null
                                    ? DecorationImage(
