...
class CabAgentPanCardUploadScreen extends StatefulWidget {
  const CabAgentPanCardUploadScreen({super.key});

  @override
  State<CabAgentPanCardUploadScreen> createState() =>
      _CabAgentPanCardUploadScreenState();
}

class _CabAgentPanCardUploadScreenState
    extends State<CabAgentPanCardUploadScreen> {
  File? _panFront;
  File? _panBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousPanFront = pref.getString("cab_agent_pan_front_photo_url");
    previousPanBack = pref.getString("cab_agent_pan_back_photo_url");
    var cab_agent_pan_no = pref.getString("cab_agent_pan_no");
    if (cab_agent_pan_no != null && cab_agent_pan_no!.isNotEmpty) {
      panNoTextEdit.text = cab_agent_pan_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("cab_agent_pan_front_photo_url", retUrl);
        } else {
          await pref.setString("cab_agent_pan_back_photo_url", retUrl);
        }

        pref.setString(
            "cab_agent_pan_no", panNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> savePanDetails() async {
    ...
    String? savedPanNo = pref.getString("cab_agent_pan_no");
    previousPanFront = pref.getString("cab_agent_pan_front_photo_url");
    previousPanBack = pref.getString("cab_agent_pan_back_photo_url");
    ...
    if (isNewEntry || savedPanNo != panNo) {
      await pref.setString("cab_agent_pan_no", panNo);
    }
