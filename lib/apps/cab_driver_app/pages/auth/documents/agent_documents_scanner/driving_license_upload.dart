...
class CabAgentDrivingLicenseUploadScreen extends StatefulWidget {
  const CabAgentDrivingLicenseUploadScreen({super.key});

  @override
  State<CabAgentDrivingLicenseUploadScreen> createState() =>
      _CabAgentDrivingLicenseUploadScreenState();
}

class _CabAgentDrivingLicenseUploadScreenState
    extends State<CabAgentDrivingLicenseUploadScreen> {
  File? _licenseFront;
  File? _licenseBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousLicenseFront = pref.getString("cab_agent_license_front_photo_url");
    previousLicenseBack = pref.getString("cab_agent_license_back_photo_url");
    var cab_agent_license_no = pref.getString("cab_agent_license_no");
    if (cab_agent_license_no != null && cab_agent_license_no!.isNotEmpty) {
      licenseNoTextEdit.text = cab_agent_license_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("cab_agent_license_front_photo_url", retUrl);
        } else {
          await pref.setString("cab_agent_license_back_photo_url", retUrl);
        }

        pref.setString(
            "cab_agent_license_no", licenseNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> saveLicenseDetails() async {
    ...
    String? savedLicenseNo = pref.getString("cab_agent_license_no");
    previousLicenseFront = pref.getString("cab_agent_license_front_photo_url");
    previousLicenseBack = pref.getString("cab_agent_license_back_photo_url");
    ...
    if (isNewEntry || savedLicenseNo != licenseNo) {
      await pref.setString("cab_agent_license_no", licenseNo);
    }
