...
class CabAgentDocumentVerificationScreen extends StatefulWidget {
  const CabAgentDocumentVerificationScreen({super.key});

  @override
  State<CabAgentDocumentVerificationScreen> createState() =>
      _CabAgentDocumentVerificationScreenState();
}

class _CabAgentDocumentVerificationScreenState
    extends State<CabAgentDocumentVerificationScreen> {
...
//     };
  ...
  Future<void> saveDriverDetails() async {
    ...
    var cab_agent_aadhar_front_photo_url =
        pref.getString("cab_agent_aadhar_front_photo_url");
    var cab_agent_aadhar_back_photo_url =
        pref.getString("cab_agent_aadhar_back_photo_url");
    var cab_agent_aadhar_no = pref.getString("cab_agent_aadhar_no");
    var cab_agent_license_front_photo_url =
        pref.getString("cab_agent_license_front_photo_url");
    var cab_agent_license_back_photo_url =
        pref.getString("cab_agent_license_back_photo_url");
    var cab_agent_license_no = pref.getString("cab_agent_license_no");
    var cab_agent_pan_no = pref.getString("cab_agent_pan_no");
    var cab_agent_pan_front_photo_url =
        pref.getString("cab_agent_pan_front_photo_url");
    var cab_agent_pan_back_photo_url =
        pref.getString("cab_agent_pan_back_photo_url");
    var cab_agent_selfie_photo_url =
        pref.getString("cab_agent_selfie_photo_url");
    ...
    if (cab_agent_license_front_photo_url == null ||
        cab_agent_license_front_photo_url.isEmpty) {
      glb.showToast("Please upload Driving License Front Picture");
      return;
    }
    if (cab_agent_license_back_photo_url == null ||
        cab_agent_license_back_photo_url.isEmpty) {
      glb.showToast("Please upload Driving License Back Picture");
      return;
    }
    if (cab_agent_license_no == null || cab_agent_license_no.isEmpty) {
      glb.showToast("Please upload Driving License Number");
      return;
    }
    if (cab_agent_aadhar_front_photo_url == null ||
        cab_agent_aadhar_front_photo_url.isEmpty) {
      glb.showToast("Please upload Aadhar Front Picture");
      return;
    }
    if (cab_agent_aadhar_back_photo_url == null ||
        cab_agent_aadhar_back_photo_url.isEmpty) {
      glb.showToast("Please upload Aadhar Back Picture");
      return;
    }
    if (cab_agent_aadhar_no == null || cab_agent_aadhar_no.isEmpty) {
      glb.showToast("Please provide your Aadhar Number");
      return;
    }
    if (cab_agent_pan_no == null || cab_agent_pan_no.isEmpty) {
      glb.showToast("Please upload PAN Number");
      return;
    }
    if (cab_agent_pan_front_photo_url == null ||
        cab_agent_pan_front_photo_url.isEmpty) {
      glb.showToast("Please upload PAN Front Picture");
      return;
    }
    if (cab_agent_pan_back_photo_url == null ||
        cab_agent_pan_back_photo_url.isEmpty) {
      glb.showToast("Please upload PAN Back Picture");
      return;
    }
    if (cab_agent_selfie_photo_url == null ||
        cab_agent_selfie_photo_url.isEmpty) {
      glb.showToast("Please upload your selfie");
      return;
    }

    pref.setString("cab_agent_driver_name", driverName);
    pref.setString("cab_agent_driver_address", driverAddress);
    pref.setString("cab_agent_driver_city_id", _selectedCityID!);
    pref.setString("cab_agent_driver_gender", _selectedGender!);
