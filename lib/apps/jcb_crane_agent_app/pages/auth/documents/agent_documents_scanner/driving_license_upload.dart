...
class JcbCraneDriverAgentDrivingLicenseUploadScreen extends StatefulWidget {
  const JcbCraneDriverAgentDrivingLicenseUploadScreen({super.key});

  @override
  State<JcbCraneDriverAgentDrivingLicenseUploadScreen> createState() =>
      _JcbCraneDriverAgentDrivingLicenseUploadScreenState();
}

class _JcbCraneDriverAgentDrivingLicenseUploadScreenState
    extends State<JcbCraneDriverAgentDrivingLicenseUploadScreen> {
  File? _licenseFront;
  File? _licenseBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    if (jcb_crane_driver_agent_license_no != null &&
        jcb_crane_driver_agent_license_no!.isNotEmpty) {
      licenseNoTextEdit.text = jcb_crane_driver_agent_license_no;
    }
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString(
              "jcb_crane_driver_agent_license_front_photo_url", retUrl);
        } else {
          await pref.setString(
              "jcb_crane_driver_agent_license_back_photo_url", retUrl);
        }

        pref.setString("jcb_crane_driver_agent_license_no",
  ...
  Future<void> saveLicenseDetails() async {
    ...
    if (isNewEntry || savedLicenseNo != licenseNo) {
      await pref.setString("jcb_crane_driver_agent_license_no", licenseNo);
    }
