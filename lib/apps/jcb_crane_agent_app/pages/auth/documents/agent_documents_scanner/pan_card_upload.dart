...
class JcbCraneDriverAgentPanCardUploadScreen extends StatefulWidget {
  const JcbCraneDriverAgentPanCardUploadScreen({super.key});

  @override
  State<JcbCraneDriverAgentPanCardUploadScreen> createState() =>
      _JcbCraneDriverAgentPanCardUploadScreenState();
}

class _JcbCraneDriverAgentPanCardUploadScreenState
    extends State<JcbCraneDriverAgentPanCardUploadScreen> {
  File? _panFront;
  File? _panBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    if (jcb_crane_driver_agent_pan_no != null &&
        jcb_crane_driver_agent_pan_no!.isNotEmpty) {
      panNoTextEdit.text = jcb_crane_driver_agent_pan_no;
    }
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString(
              "jcb_crane_driver_agent_pan_front_photo_url", retUrl);
        } else {
          await pref.setString(
              "jcb_crane_driver_agent_pan_back_photo_url", retUrl);
        }

        pref.setString("jcb_crane_driver_agent_pan_no",
            panNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> savePanDetails() async {
    ...
    String? savedPanNo = pref.getString("jcb_crane_driver_agent_pan_no");
    previousPanFront =
        pref.getString("jcb_crane_driver_agent_pan_front_photo_url");
    previousPanBack =
        pref.getString("jcb_crane_driver_agent_pan_back_photo_url");
    ...
    if (isNewEntry || savedPanNo != panNo) {
      await pref.setString("jcb_crane_driver_agent_pan_no", panNo);
    }
