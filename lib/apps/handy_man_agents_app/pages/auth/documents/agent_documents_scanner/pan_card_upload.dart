...
class HandyManAgentPanCardUploadScreen extends StatefulWidget {
  const HandyManAgentPanCardUploadScreen({super.key});

  @override
  State<HandyManAgentPanCardUploadScreen> createState() =>
      _HandyManAgentPanCardUploadScreenState();
}

class _HandyManAgentPanCardUploadScreenState
    extends State<HandyManAgentPanCardUploadScreen> {
  File? _panFront;
  File? _panBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    previousPanFront = pref.getString("handy_man_agent_pan_front_photo_url");
    previousPanBack = pref.getString("handy_man_agent_pan_back_photo_url");
    var handy_man_agent_pan_no = pref.getString("handy_man_agent_pan_no");
    if (handy_man_agent_pan_no != null && handy_man_agent_pan_no!.isNotEmpty) {
      panNoTextEdit.text = handy_man_agent_pan_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("handy_man_agent_pan_front_photo_url", retUrl);
        } else {
          await pref.setString("handy_man_agent_pan_back_photo_url", retUrl);
        }

        pref.setString(
            "handy_man_agent_pan_no", panNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> savePanDetails() async {
    ...
    String? savedPanNo = pref.getString("handy_man_agent_pan_no");
    previousPanFront = pref.getString("handy_man_agent_pan_front_photo_url");
    previousPanBack = pref.getString("handy_man_agent_pan_back_photo_url");
    ...
    if (isNewEntry || savedPanNo != panNo) {
      await pref.setString("handy_man_agent_pan_no", panNo);
    }
