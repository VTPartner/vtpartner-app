...
class HandyManAgentDrivingLicenseUploadScreen extends StatefulWidget {
  const HandyManAgentDrivingLicenseUploadScreen({super.key});

  @override
  State<HandyManAgentDrivingLicenseUploadScreen> createState() =>
      _HandyManAgentDrivingLicenseUploadScreenState();
}

class _HandyManAgentDrivingLicenseUploadScreenState
    extends State<HandyManAgentDrivingLicenseUploadScreen> {
  File? _licenseFront;
  File? _licenseBack;
  ...
  Future<void> _setupCameraController() async {
    ...
    if (handy_man_agent_license_no != null &&
        handy_man_agent_license_no!.isNotEmpty) {
      licenseNoTextEdit.text = handy_man_agent_license_no;
    }
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString(
              "handy_man_agent_license_front_photo_url", retUrl);
        } else {
          await pref.setString(
              "handy_man_agent_license_back_photo_url", retUrl);
        }

        pref.setString("handy_man_agent_license_no",
  ...
  Future<void> saveLicenseDetails() async {
    ...
    String? savedLicenseNo = pref.getString("handy_man_agent_license_no");
    previousLicenseFront =
        pref.getString("handy_man_agent_license_front_photo_url");
    previousLicenseBack =
        pref.getString("handy_man_agent_license_back_photo_url");
    ...
    if (isNewEntry || savedLicenseNo != licenseNo) {
      await pref.setString("handy_man_agent_license_no", licenseNo);
    }
