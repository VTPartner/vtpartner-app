...
class _PanCardUploadScreenState extends State<PanCardUploadScreen> {
  ...
  Future<void> _setupCameraController() async {
    ...
    previousPanFront = pref.getString("pan_front_photo_url");
    previousPanBack = pref.getString("pan_back_photo_url");
    var pan_no = pref.getString("pan_no");
    if (pan_no != null && pan_no!.isNotEmpty) {
      panNoTextEdit.text = pan_no;
    }
  ...
  Future<void> _uploadImage(File image, bool isFront) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isFront) {
          await pref.setString("pan_front_photo_url", retUrl);
        } else {
          await pref.setString("pan_back_photo_url", retUrl);
        }

        pref.setString("pan_no", panNoTextEdit.text.toString().trim());
        glb.showToast(
  ...
  Future<void> savePanDetails() async {
    ...
    String? savedPanNo = pref.getString("pan_no");
    previousPanFront = pref.getString("pan_front_photo_url");
    previousPanBack = pref.getString("pan_back_photo_url");
    ...
    if (isNewEntry || savedPanNo != panNo) {
      await pref.setString("pan_no", panNo);
    }
